...
**The mapping file.** `beads-map.jsonl` (`--mapping` to choose another) records every created issue as it happens, flushed per line: `{"bead":"wx-1","number":101,"url":"…","phase":"created","comments":0}`, one line per step, the last line per bead winning. `phase` is `created` (the issue exists; `comments` says how many of its comments are on) or `done`. Keep it: it resolves `bd-xxxx` references in old docs and commit messages, and mentions of a mapped id inside imported bodies and comments are rewritten to `#n` (a reference to a bead created later in the same run stays as it was). Re-running with the same file resumes: done beads are skipped, partially imported ones are finished, and nothing recorded is created twice. Ctrl-C is safe with one caveat: a line is written right after each create and each comment, so an interrupt in that instant leaves one issue or one comment on GitHub that the file does not know about. Before resuming after a Ctrl-C, check two things: the newest issue in the repo is in the file (if not, append its `created` line with `"comments":0`), and the comment count on the last issue the file mentions matches that line's `comments` (if GitHub has one more, append a `created` line with the higher count). The file is tied to the repository it was written for and refused elsewhere.
